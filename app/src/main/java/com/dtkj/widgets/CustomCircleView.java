...
public class CustomCircleView extends View {
    ...
    public CustomCircleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray type = context.obtainStyledAttributes(attrs, R.styleable.custom_circle_view);
        mBorderColor = type.getColor(R.styleable.custom_circle_view_mborder_color, 0);
        mDrawable = type.getDrawable(R.styleable.custom_circle_view_msrc);

        //将获得的 Drawable 转换成 Bitmap
        BitmapDrawable bitmapDrawable = (BitmapDrawable) mDrawable;
        mBitmap = bitmapDrawable.getBitmap();

        mBorderWidth = type.getDimensionPixelSize(R.styleable.custom_circle_view_mborder_width, 2);
    }
